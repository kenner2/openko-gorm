...
type CouponSerialList struct {
	...
	SerialNum  []byte `gorm:"column:strSerialNum;type:char(16);not null" json:"strSerialNum"`
...
func (this CouponSerialList) GetInsertData() string {
	return fmt.Sprintf("(%s, CONVERT(char(16), %s), %s, %s)", GetOptionalDecVal(&this.Index),
		GetOptionalByteArrayVal(&this.SerialNum, true),
		GetOptionalDecVal(&this.ItemNumber),
		GetOptionalDecVal(&this.ItemCount))
}
...
func (this CouponSerialList) GetAllTableData(db *gorm.DB) (results []Model, err error) {
	...
	rawSql := "SELECT [nIndex], CONVERT(VARBINARY(16), [strSerialNum]) as [strSerialNum], [nItemNum], [sItemCount] FROM [COUPON_SERIAL_LIST]"
...
var _CouponSerialListSelectClause = clause.Select{
	Columns: []clause.Column{
		...
		clause.Column{
			Name: "CONVERT(VARBINARY(16), [strSerialNum]) as [strSerialNum]",
		},
		clause.Column{
			Name: "[nItemNum]",
		},
